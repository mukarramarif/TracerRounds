...
const std::string TEXTURE_PATH = "textures/M4A1Diffuse.png";
...
private:
	...
	//bullet members 
    const std::string BULLET_MODEL_PATH = "models/bullet.obj";

    std::vector<Vertex> bulletVertices;
    std::vector<uint32_t> bulletIndices;
        ...
        for (const auto& shape : shapes) {
            for (const auto& index : shape.mesh.indices) {
                ...
                if (uniqueVertices.count(vertex) == 0) {
                    ...
                    bulletVertices.push_back(vertex);
                }
    ...
    void createVertexBuffer(const std::vector<Vertex>& vertices, VkBuffer& vertexBuffer, VkDeviceMemory& vertexBufferMemory) {
        ...
        // Create the actual vertex buffer in device-local memory
        createBuffer(bufferSize,
    ...
    void createIndexBuffer(const std::vector<uint32_t>& indices, VkBuffer& indexBuffer, VkDeviceMemory& indexBufferMemory) {
        ...
        // Create the actual index buffer in device-local memory
        createBuffer(bufferSize,
    ...
    void recordCommandBufferWithImGui(VkCommandBuffer commandBuffer, uint32_t imageIndex, ImDrawData* draw_data) {
        VkCommandBufferBeginInfo beginInfo{};
        beginInfo.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_BEGIN_INFO;
